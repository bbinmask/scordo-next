...
const statusConfig = {
  ...
  completed: {
    label: "Completed",
    cls: "bg-slate-500/10 text-slate-400 border-slate-200",
    dot: false,
  },
  stopped: {
    label: "Stopped",
    ...
    <Link href={`/matches/${match.id}`}>
      <div className="hover-card group flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 transition-all dark:border-white/10 dark:bg-slate-900/50">
        ...
        <div className="hidden shrink-0 text-right md:block">
          {match.result ? (
            <p className="max-w-[180px] truncate font-[urbanist] text-xs font-semibold text-slate-500 dark:text-slate-400">
              {match.result}
            </p>
          ) : match.date ? (
        ...
        <div className="shrink-0">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 font-[urbanist] text-[10px] font-black tracking-widest uppercase ${cfg.cls}`}
          >
  ...
  const groups = [
    {
      key: "live",
      label: "Live",
      items: matches.filter((m) => m.status === "in_progress" || m.status === "inning_completed"),
    },
    {
      key: "upcoming",
      label: "Upcoming",
      items: matches.filter((m) => m.status === "not_started"),
    },
    {
      key: "completed",
      label: "Completed",
      items: matches.filter((m) => m.status === "completed"),
    },
  ...
  return (
    <div className="space-y-8">
      {groups.map(({ key, label, items }) => (
        <div key={key}>
          <h4 className="mb-3 font-[poppins] text-xs font-black tracking-widest text-slate-400 uppercase">
            {label} · {items.length}
          </h4>
